...
\subsection{Iterative DP (implemented)}
...
Implemented behind \texttt{GCSA\_PFDP\_ITERATE=1} (off by default): each further round re-extracts, over the still-unresolved words only, every remaining word's currently \emph{available} best candidate (given what earlier rounds already accepted) and solves that round's pseudoforest exactly again, same as round 0. The loop stops at a fixed point -- a round whose DP accepts nothing -- or at \texttt{GCSA\_PFDP\_MAX\_ROUNDS} if set; it needs no cap to terminate on its own, since a productive round always accepts at least one more word.

This turned out \emph{not} to be a strict improvement over one DP round followed by the leftover pass, which is worth recording since it runs against the intuition that repeating an exact solve can only help. Each round is exact only against its own single up-front snapshot -- one fixed candidate per still-unresolved word, chosen once at the start of that round -- and a word whose candidate loses a conflict inside the round cannot fall back to its second-best candidate until an entirely new round is run. The plain greedy leftover pass has no such restriction: it re-derives "the best still-available candidate" one word at a time against always-current state, so it can fall back to a second choice mid-pass in a way no single DP round can. Measured on the \texttt{bench/} E.\ coli genomes across several shapes and \texttt{--max-add} values, the end-to-end effect (through leftover, and again through Phase II) is a coin flip: usually no change, otherwise a difference of a handful of $|C|$ positions out of several million kept, in either direction -- never large, and never consistently favoring iteration. It is left opt-in pending a wider sweep.
